...
export const addProperty = async (req, res) => {
  try {
    ...
    const property = await Property.create({
      ...
      images: imageUrls,
      seller: req.user._id, // only seller can create the property
      amenities: req.body.amenities
        ? Array.isArray(req.body.amenities)
          ? req.body.amenities
          : (() => {
            try {
              return JSON.parse(req.body.amenities); // for '["WiFi","Parking"]'
            } catch (e) {
              return req.body.amenities.split(","); // for "Wifi,Parking"
            }
          })() // this empty bracket means execute this function immediately
        : [],
    });
    ...
    console.log("ADD_PROPERTY_ERROR : ", err);
    res.status(500).json({
      success: false,
      message: err.message || "Internal sever error while creating property"
    })
  }
}
...
export const getMyProperties = async (req, res) => {
  try {
    const properties = await Property.find({ seller: req.user._id }).sort({ createdAt: -1 }); // Descending order
    res.json({
      success: true,
      properties,
    });
...
export const updateProperty = async (req, res) => {
  try {
    ...
    // for image handling
    if (req.body.existingImages) { // contains existing images
      try {
    ...
    if (req.files && req.files.length > 0) { // containes new files/images
      let newImages = [];
      for (let file of req.files) {
...
export const getAllProperties = async (req, res) => {
  try {
    ...
    if (propertyType) {
      ...
      // $in means: Match the document if the field's value is one of these values
    }
    if (bhk) {
      if (bhk === "5+") {
        query.bhk = { $gte: "5" }; // $gte means: greater than or equal to
      } else {
        query.bhk = bhk;
      }
    }
...
export const getPropertyDetails = async (req, res) => {
  try {
    ...
    let visitedId = req.ip;
    const authHeader = req.headers.authorization;
    if (authHeader && authHeader.startsWith("Bearer ")) {
      try {
        const token = authHeader.split(" ")[1];
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        visitedId = decoded.id; // stores the buyer user id
      }
      catch (error) {
        //ignore    
      }
